  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <FormField label="Opening hours" error={errors.openingHours}>
            <TextInput {...register('openingHours')} />
          </FormField>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <FormField label="Image URL" error={errors.image}>
            <TextInput {...register('image')} />
          </FormField>
          <FormField label="Gallery URLs (comma separated)">
            <TextInput {...register('gallery')} />
          </FormField>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <FormField label="Rating" error={errors.rating}>
            <TextInput type="number" min={0} max={5} step={0.1} {...register('rating', { valueAsNumber: true })} />
          </FormField>
          <FormField label="Price" error={errors.price}>
            <TextInput type="number" min={0} step={0.01} {...register('price', { valueAsNumber: true })} />
          </FormField>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <FormField label="Longitude">
            <TextInput type="number" step="0.000001" {...register('geometry.coordinates.0', { valueAsNumber: true })} />
          </FormField>
          <FormField label="Latitude">
            <TextInput type="number" step="0.000001" {...register('geometry.coordinates.1', { valueAsNumber: true })} />
          </FormField>
        </div>
